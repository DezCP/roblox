...
checkDriver = False
checkScriptVersion = False

disablePredictor = True
...
class main:
	...
	def getConfig(self): # Get configuration from config.json file
		...
		with open("config.json", "r+") as data:
			...
			if checkDriver: self.installDriver()
		...
		for game in self.ChrashPoints():
			...
			try:
				if lastgame >= prediction:
					...
					uiprint(f"Accuracy on last {'guess' if not disablePredictor else 'bet'}: {accuracy}", "yellow")
					try:
					...
					uiprint(f"Accuracy on previous {'guess' if not disablePredictor else 'bet'}: {accuracy}", "yellow")
					try:
						self.updateBetAmount(betamount)
					except Exception as e:
						uiprint(f"Input box for bet amount not found: {e}\n Redirecting to crash page", "error")
